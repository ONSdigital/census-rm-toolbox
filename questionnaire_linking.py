...
def validate_and_submit_questionnaire_links(qid_link_file_reader):
    ...
    for line_number, questionnaire_link in enumerate(qid_link_file_reader, 1):
        ...
        case_id = validate_case_id(line_number, questionnaire_link["case_ref"])

        if error_count > 0:
            print(f'{questionnaire_link} on line {line_number} has FAILED')
        else:
            post_message_to_queue(case_id, questionnaire_link["qid"])
        print("\n")
...
def validate_check_digits(line_number, qid):
    ...
    if not qid_checksum_validator.validate(qid, int(Config.QID_MODULUS), int(Config.QID_FACTOR))[0]:
        print(f'Check digits incorrect for QID {qid}, line {line_number}')
        error_count += 1
...
def validate_case_id(line_number, case_ref):
    ...
    if response.status_code == 404:
        print(f'Error: Case_ref {case_ref} on line {line_number} does not exist')
        error_count += 1
        return None
...
def post_message_to_queue(case_id, qid):
    message = {
        "event": {
    ...
    with RabbitContext() as rabbit:
        ...
        print(f'Case Reference {case_id} and QID {qid} have PASSED')
